...
public class Player extends GameObject {
    ...
    public Rectangle getBounds(){
        return new Rectangle (this.x+(w/2)-((w/2)/2),this.y+(this.h/2),this.w/2,this.h/2);
    }
    //This gets the top bounds of the rectangle
    public Rectangle getPBoundsTop(){
        return new Rectangle (this.x+(w/2)-((w/2)/2),this.y,this.w/2,this.h/2);
    }
    //This gets the right bounds of the rectangle
    public Rectangle getPBoundsR(){
        return new Rectangle (this.x+w-5,this.y+5,5,this.h-10);
    }
    //This gets the left bounds of the rectangle
    public Rectangle getPBoundsL(){
        return new Rectangle (this.x,this.y+5,5,this.h-10);
    }
    ...
    public void collision(LinkedList<GameObject> object){
        for(int i=0;i<handler.object.size();i++){
            ...
            if(temp.getID()==ID.Platform || temp.getID()==ID.Enemy){
                ...
                //else{ System.out.println("Damn Son, we missed"); }
            }
        }
    }
